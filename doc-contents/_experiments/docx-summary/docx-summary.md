...
## DOCX 1
The next example demonstrates a generated summary without any subsequent manual correction. Observe that the quality of the summary is much better than the summary generated for the pdf version of the same document. Except for a small correction in the last paragraph (sometimes the model does not interpret well the abbreviations), the docx summary do not require heavy manual corrections.
...
{% include elements/alert.html class="primary" content="Since summaries can be pretty long sometimes, we recommend to set a fixed height to increase the readability of the document and the UX when reading." title="Tip" %}