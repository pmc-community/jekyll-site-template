...
const goToAnchor = () => {
    ...
    const $anchor = $(hash);
    const headerHeight = $(settings.headerAboveContent.headerID).height() || 0;
    const offset = settings.headerAboveContent.offsetWhenScroll + 100;
    const maxRetries = 10;
    const retryDelay = 100; // ms

    const scrollToTarget = (retryCount = 0) => {
        const targetTop = $anchor.offset().top - headerHeight - offset;
        ...
        // Retry if we're not close enough (layout might not be finished)
        setTimeout(() => {
            const newTargetTop = $anchor.offset().top - headerHeight - offset;
            const newScroll = $(window).scrollTop();

            if (Math.abs(newScroll - newTargetTop) > 5 && retryCount < maxRetries) {
